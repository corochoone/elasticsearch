...
The index operation also accepts an `op_type` parameter, that can be used to force
a `create` operation, allowing for "put-if-absent" behavior. When
`create` value is used, the index operation will fail if a document with specified id
...
Another way to specify `create` is to use the following URI:
...
case, an id will be generated automatically. In addition, the `op_type` parameter
will automatically be set to `create` value. Here is an example (note the
...
In the example above, the "tweet" document is routed to a shard specified
by `routing` parameter with "kimchy" value.
...
If the `timestamp` parameter value is not provided externally or in the `_source`,
...
it, using `ttl` parameter. Expired documents will be expunged from
index automatically. The expiration date that will be set for a
document with a provided `ttl` is relative to the `timestamp`
of the document, meaning it can be based on the time
of indexing or on any time provided. The provided `ttl` value must be strictly
